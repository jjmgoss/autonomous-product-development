...
DEFAULTS_PATH = ROOT / "theme.md"

OPTIONAL_EMPTY_VALUES = {"", "none", "n/a", "na", "null"}
KICKOFF_SLUG_STOPWORDS = {
    "a",
    "an",
    "and",
    "autopd",
...
RUN_MODE_PROFILES = {
    "test": {
        "label": "compact validation run",
        "minimum_sources": 6,
        "maximum_sources": 12,
        "minimum_source_types": 3,
        "maximum_candidates": 5,
        "maximum_scored_candidates": 3,
        "completion_expectation": "Reach a compact, completion-checked package and continue into a lightweight prototype-planning handoff when one candidate clearly wins.",
    },
    "real": {
        "label": "deep execution run",
        "minimum_sources": 10,
        "maximum_sources": 24,
        "minimum_source_types": 4,
        "maximum_candidates": 7,
        "maximum_scored_candidates": 4,
        "completion_expectation": "Keep researching until the ranking is genuinely credible, then continue further into product brief, requirements, design, prototype planning, and backlog unless a hard boundary applies.",
    },
}
...
def checkpoint_label() -> str:
    return parse_active_run_field("checkpoint label") or "Checkpoint 1"


def checkpoint_behavior() -> str:
    return parse_active_run_field("checkpoint behavior") or "status marker only; continue by default"


def extract_defaults_summary() -> str:
    if not DEFAULTS_PATH.exists():
        return "No defaults file configured."

    text = read_text(DEFAULTS_PATH)
    ...
    for line in text.splitlines():
        ...
        if stripped and not stripped.startswith("#"):
            return stripped
    return "Repository defaults present, but no summary line was found."
...
def infer_slug_from_intent(intent: str) -> str:
    tokens = [
        token
        for token in re.findall(r"[a-z0-9]+", intent.lower())
        if token not in KICKOFF_SLUG_STOPWORDS
    ]
    selected = tokens[:4] or re.findall(r"[a-z0-9]+", intent.lower())[:4] or ["run"]
...
def mode_profile(mode: str) -> dict[str, int | str]:
    return RUN_MODE_PROFILES[mode]


def kickoff_command(mode: str, intent: str) -> str:
    return f'autopd {mode} "{intent}"'


def research_manifest_stub(run_id: str, mode: str, intent: str) -> dict:
    ...
    return {
        ...
        "mode": mode,
        "intent": intent,
        "kickoff_command": kickoff_command(mode, intent),
        "defaults_reference": "theme.md",
        "completion_note": "Replace this scaffold metadata with saved source entries before running the completion check.",
        "source_requirements": {
            "minimum_sources": profile["minimum_sources"],
            "maximum_sources": profile["maximum_sources"],
            "minimum_source_types": profile["minimum_source_types"],
            "required_fields": RESEARCH_MANIFEST_FIELDS,
        },
        "candidate_requirements": {
            "maximum_candidates": profile["maximum_candidates"],
            "maximum_scored_candidates": profile["maximum_scored_candidates"],
        },
        "sources": [],
    }


def artifact_manifest_stub(run_id: str, mode: str, intent: str) -> dict:
    return {
        "run_id": run_id,
        "population_status": LAUNCH_SCAFFOLD_MARKER,
        "mode": mode,
...
def candidate_links_stub(run_id: str, mode: str, intent: str) -> str:
    profile = mode_profile(mode)
    return dedent(
        f"""\
...
def run_index_stub(run_id: str, mode: str, intent: str) -> str:
    profile = mode_profile(mode)
    return dedent(
        f"""\
        # Run Index

        Launch scaffold: this run has been initialized, but the package is not complete yet.
        ...
        - kickoff command: {kickoff_command(mode, intent)}
        - intent: {intent}
        - defaults reference: theme.md
        - defaults summary: {extract_defaults_summary()}
        - corpus path: research-corpus/runs/{run_id}/
        - artifact path: artifacts/runs/{run_id}/
        - checkpoint label: {checkpoint_label()}
        - checkpoint behavior: {checkpoint_behavior()}

        ## Boundary Status

        - source target: {profile['minimum_sources']}-{profile['maximum_sources']} meaningful sources
        - actual source count: 0
        - actual source-type count: 0
        - candidate target: at most {profile['maximum_candidates']} serious candidates, score at most top {profile['maximum_scored_candidates']} in detail
        ...
        - manifest is populated with real saved source entries: no
        - candidate-evidence map is populated with supporting and weakening evidence IDs: no
        - artifact manifest is populated with completed artifact entries: no
        - review-package files are fully written rather than template-shaped: no
        - completion check result: not run yet
        ...
        - status marker: {checkpoint_label()} not reached yet
        - hard-boundary status: none triggered yet
        - completion point status: run work still in progress
        """
    )


def research_review_stub(run_id: str, mode: str, intent: str) -> str:
    profile = mode_profile(mode)
    return dedent(
        f"""\
        # Research
        ...
        - source expectation: {profile['minimum_sources']}-{profile['maximum_sources']} sources across at least {profile['minimum_source_types']} source types
...
def opportunity_scorecard_stub(run_id: str, mode: str, intent: str) -> str:
    profile = mode_profile(mode)
    return dedent(
        f"""\
        ...
        Score at most the top {profile['maximum_scored_candidates']} candidates in detail. Cite saved evidence IDs in each judgment.
...
def candidate_review_stub(run_id: str, mode: str, intent: str) -> str:
    return dedent(
        f"""\
        # Candidate Review
...
def validation_stub(run_id: str, mode: str, intent: str) -> str:
    return dedent(
        f"""\
        # Validation
...
def discovery_summary_stub(run_id: str, mode: str, intent: str) -> str:
    return dedent(
        f"""\
        # Discovery Summary
        ...
        State whether the package reached {checkpoint_label()}, what the recommended next stage is, whether any hard boundary applies, and whether the loop should continue.
        """
    )
...
def initialize_run(run_id: str, mode: str, intent: str) -> tuple[Path, Path]:
    ...
    (research_root / "manifest.json").write_text(
        json.dumps(research_manifest_stub(run_id, mode, intent), indent=2) + "\n",
        encoding="utf-8",
    )
    write_scaffold(research_root / "candidate-links.md", candidate_links_stub(run_id, mode, intent))
    (artifact_root / "manifest.json").write_text(
        json.dumps(artifact_manifest_stub(run_id, mode, intent), indent=2) + "\n",
        encoding="utf-8",
    )
    write_scaffold(artifact_root / "run-index.md", run_index_stub(run_id, mode, intent))
    write_scaffold(artifact_root / "review-package" / "research.md", research_review_stub(run_id, mode, intent))
    write_scaffold(artifact_root / "review-package" / "opportunity-scorecard.md", opportunity_scorecard_stub(run_id, mode, intent))
    write_scaffold(artifact_root / "review-package" / "candidate-review.md", candidate_review_stub(run_id, mode, intent))
    write_scaffold(artifact_root / "review-package" / "validation.md", validation_stub(run_id, mode, intent))
    write_scaffold(artifact_root / "reports" / "discovery-summary.md", discovery_summary_stub(run_id, mode, intent))

    return research_root, artifact_root


def parse_intent(args: argparse.Namespace) -> str:
    parts: list[str] = []
    if args.intent:
        parts.append(args.intent.strip())
    if args.intent_parts:
        parts.append(" ".join(args.intent_parts).strip())

    intent = " ".join(part for part in parts if part).strip()
    if not intent:
        raise ValueError("Kickoff intent is required. Use `python scripts/autopd.py test \"<intent>\"` or `python scripts/autopd.py real \"<intent>\"`.")
    return intent


def main() -> int:
    parser = argparse.ArgumentParser(description="Launch a direct-intent autonomous product-development run.")
    parser.add_argument("mode", choices=sorted(RUN_MODE_PROFILES), help="Run mode: compact `test` or deeper `real`.")
    parser.add_argument("intent_parts", nargs="*", help="Direct user intent written inline after the mode.")
    parser.add_argument("--intent", help="Explicit direct intent, if you do not want to pass it positionally.")
    parser.add_argument("--slug", help="Override the default intent-derived slug.")
    parser.add_argument("--run-id", help="Explicit run ID to use. Fails if it already exists.")
    ...
    if args.run_id:
        ...
        slug = slugify(args.slug) if args.slug else infer_slug_from_intent(intent)
        run_id = next_run_id(date_prefix, slug)

    research_root, artifact_root = initialize_run(run_id, args.mode, intent)
    profile = mode_profile(args.mode)

    print(f"RUN_ID  {run_id}")
    print(f"MODE  {args.mode}")
    print(f"MODE_PROFILE  {profile['label']}")
    print(f"INTENT  {intent}")
    print(f"KICKOFF_COMMAND  {kickoff_command(args.mode, intent)}")
    print(f"DEFAULTS_REFERENCE  {DEFAULTS_PATH.relative_to(ROOT).as_posix() if DEFAULTS_PATH.exists() else 'none'}")
    print(f"DEFAULTS_SUMMARY  {extract_defaults_summary()}")
    ...
    print(f"SOURCE_TARGET  {profile['minimum_sources']}-{profile['maximum_sources']}")
    print(f"SOURCE_TYPE_TARGET  {profile['minimum_source_types']}")
    print(f"CANDIDATE_TARGET  <= {profile['maximum_candidates']}")
    print(f"SCORED_CANDIDATE_TARGET  <= {profile['maximum_scored_candidates']}")
    print(f"COMPLETION_EXPECTATION  {profile['completion_expectation']}")
    print("NEXT_STEP  Launch complete. Do the actual run work now, fully populate the corpus, manifests, candidate links, and reviewer artifacts, then run the completion check last.")
