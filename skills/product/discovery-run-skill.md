...
Run a bounded discovery pass that ends with a fully populated Gate 1 package.
...
5. Save each meaningful source and update `research-corpus/runs/<run-id>/manifest.json` as you go.
...
10. Complete `artifacts/runs/<run-id>/run-index.md` as the reviewer control document.
11. Run `python scripts/check_repo_readiness.py --run-id <run-id>` only after the package is complete.
12. Stop at Gate 1 unless a human explicitly approves a later stage.
...
- the candidate map is thin or missing weakening evidence
...
- collecting sources without updating the manifest
