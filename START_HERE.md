...
2. Read the boundary file and detailed prompt named there. If the current task is build-forward from an already completed run, also read `BUILD_RUN.md`.
...
9. If build-forward mode is active, use `docs/prototype-standard.md` and the shared prototype scaffold before inventing new project structure.
10. Continue into the next non-risky stage by default unless `ACTIVE_RUN.md` says the current completion point ends the run.

## Execution Rules

- Treat `ACTIVE_RUN.md` as the canonical run selector.
- Treat `BUILD_RUN.md` as the canonical boundary file for build-forward work after a selected wedge is already known.
...
- Treat `docs/prototype-standard.md` as the default quality bar for any local prototype.
- Prefer the shared scaffold under `artifacts/shared/prototype-scaffold/` when a build-forward task needs a fresh local app shape.
...
Prototype, hardening, and local polish work are not hard boundaries by themselves.
